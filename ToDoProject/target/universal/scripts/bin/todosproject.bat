@REM todosproject launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM TODOSPROJECT_config.txt found in the TODOSPROJECT_HOME.
@setlocal enabledelayedexpansion

@echo off


if "%TODOSPROJECT_HOME%"=="" (
  set "APP_HOME=%~dp0\\.."

  rem Also set the old env name for backwards compatibility
  set "TODOSPROJECT_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%TODOSPROJECT_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\TODOSPROJECT_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running todosproject.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !TODOSPROJECT_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
